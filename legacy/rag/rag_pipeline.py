...
Flow:
    ...
    Query Rewriting & Context Resolution (Resolves follow-ups into standalone queries)
        ↓
    Multi-Document Hybrid Candidate Retrieval (ChromaDB Vector + BM25 Lexical + Exact Boost)
        ↓
...
from rag.reranker import rerank_chunks, calculate_groundedness_score
...
from rag.logger import log_pipeline_event
...
def _build_evidences_and_sources(final_results):
...
def _parse_answer_structure(raw_text: str, evidences: list, sources: list):
