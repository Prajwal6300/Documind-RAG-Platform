...
def _get_gemini_client():
    ...
    if _gemini_client is None:
        try:
            ...
            print(f"[DocuMind Embeddings] Failed to init Gemini client: {e}")
            return None
...
def embed_documents(documents: list[str]) -> list[list[float]]:
    ...
    if client is not None:
        try:
            ...
            print(f"[DocuMind Embeddings] Gemini embed failed ({exc}), falling back to local model.")
...
def embed_query(query: str) -> list[float]:
    ...
    if client is not None:
        try:
            ...
            print(f"[DocuMind Embeddings] Gemini query embed failed ({exc}), falling back to local.")

    local = _get_local_model()
    raw = local.encode(query.strip(), normalize_embeddings=True)
    return raw.tolist()
