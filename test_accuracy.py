...
def main():
    ...
    check(
        ...
        any(s["source"] == "resume.txt" for s in sources),
        f"got: {sources}",
    )
    ...
    check(
        ...
        any(s["source"] == "company_policy.pdf" for s in sources),
        f"got: {sources}",
    )
    print(f"  ({time.time()-t0:.1f}s)")
    ...
    check(
        ...
        any(s["source"] == "employee_handbook.pdf" for s in sources),
        f"got: {sources}",
    )
    print(f"  ({time.time()-t0:.1f}s)")
    ...
    check(
        ...
        any(s["source"] == "inventory.docx" for s in sources),
        f"got: {sources}",
    )
    print(f"  ({time.time()-t0:.1f}s)")
    ...
    if doc_id:
        ...
        check(
            ...
            all(s["source"] == "resume.txt" for s in sources) is True,
            f"got: {sources}",
        )
        print(f"  ({time.time()-t0:.1f}s)")
