...
def strip_question_boilerplate(question):
...
def detect_query_intents(question):
...
def extract_keywords(question, max_keywords=8):
...
def expand_query(question, keywords=None, entities=None):
...
def resolve_follow_up(question, chat_history):
