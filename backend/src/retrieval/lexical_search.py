...
def normalize_code_or_entity(val: str) -> str:
...
class BM25Index:
    ...
    def score_query(self, query_tokens: list[str]) -> list[tuple[dict, float]]:
...
def calculate_exact_match_boost(query_text: str, entities: list, chunk_text: str) -> float:
...
def calculate_keyword_overlap(query_tokens: list[str], chunk_text: str) -> float:
